...
Generates synthetic Form 16 and Form 26AS images, drives the full docs-first API
flow (upload/extract -> analyze-gaps -> submit-gaps -> reconcile -> consolidate
-> compute -> guidance), and prints a summary. Requires the backend running on
API_BASE with a valid OPENAI_API_KEY.
...
def _wait_for_docs(sid: str, expected: int, timeout: float = 90.0) -> dict:
    """Poll session state until ``expected`` documents have finished extracting."""
    deadline = time.time() + timeout
    while time.time() < deadline:
        state = requests.get(f"{API}/api/session/{sid}/state").json()
        docs = state.get("documents", {})
        count = sum(len(v) if isinstance(v, list) else 1 for v in docs.values())
        if count >= expected:
            return state
        time.sleep(2)
    raise TimeoutError("documents did not finish extracting in time")
...
def main() -> None:
    ...
    # Upload both (extraction runs in the background).
    ...
    _wait_for_docs(sid, expected=2)
    print("documents extracted")

    gaps = requests.post(f"{API}/api/session/{sid}/analyze-gaps").json()
    print("regime:", gaps["regime"], "| inferred:", gaps["inferred"])

    decision = requests.post(f"{API}/api/session/{sid}/submit-gaps", json={
        "age": 31, "residential_status": "resident", "num_house_properties": 0,
    }).json()["decision"]
    print("form:", decision["form"])

    recon = requests.post(f"{API}/api/session/{sid}/reconcile").json()
    print("reconcile issues:", len(recon["issues"]))

    requests.post(f"{API}/api/session/{sid}/consolidate")
